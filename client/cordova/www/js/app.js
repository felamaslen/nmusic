...
/***/ function(module, exports, __webpack_require__) {
	...
	var _reducersMainReducerJs = __webpack_require__(192);
	
	var _reducersMainReducerJs2 = _interopRequireDefault(_reducersMainReducerJs);
	
	var _effectsHandlersApiCallEffectHandler = __webpack_require__(195);
	
	var _effectsHandlersApiCallEffectHandler2 = _interopRequireDefault(_effectsHandlersApiCallEffectHandler);
	
	// import Component from './Component'
	
	var Reduction = new _immutable.Record({
	  appState: (0, _immutable.fromJS)({
	    loaded: false,
	    loadedOnLastRender: false,
	    something: _config.TEST_CONFIG,
	    playing: false
	  }),
	...
	var App = (function (_Component) {
	  ...
	  function App(props) {
	    ...
	    dispatcher.register(function (action) {
	      var reduction = _this.state.reduction;
	
	      // let's store all actions so that we can replay them
	      var actionLog = _this.state.actionLog.push(action);
	
	      // we want to purge list of effects before every action
	      reduction = reduction.set('effects', _immutable.List.of());
	
	      // all reducers are being executed here
	      reduction = (0, _reducersMainReducerJs2['default'])(reduction, action);
	
	      // all effect handlers are being handled here
	      reduction.get('effects').forEach(_effectsHandlersApiCallEffectHandler2['default'].bind(null, dispatcher));
	
	      // let's set the reduction back to the Component's state,
	      // this will result in re-render of those pure views, whose
	      // props have changed.
	      _this.setState({ reduction: reduction, actionLog: actionLog });
	    });
	  ...
	  _createClass(App, [{
	    ...
	    value: function componentDidUpdate() {
	      ...
	      window.replay = function () {
	        ...
	        var reduction = _this2.state.actionLog.reduce(_reducersMainReducerJs2['default'], new Reduction()).set('effects', _immutable.List.of());
	
	        _this2.setState({ reduction: reduction });
	      };
	    }
	  }, {
	    key: 'render',
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _defineProperty = __webpack_require__(193)['default'];
	...
	var _constantsActions = __webpack_require__(194);
	
	var buildReducer = function buildReducer(handlers) {
	  return function (reduction, action) {
	    return (0, _immutable.Map)(handlers).filter(function (handler, actionType) {
	      return actionType === action.type;
	    }).reduce(function (partialReduction, handler) {
	      return handler(partialReduction, action.payload);
	    }, reduction);
	  };
	};
	
	// This is the place where all magic belongs
	exports['default'] = buildReducer(_defineProperty({}, _constantsActions.APP_HIDE_SPINNER, function (reduction) {
	  return reduction.setIn(['appState', 'loadedOnLastRender'], true);
	}));
	module.exports = exports['default'];
...
/***/ function(module, exports) {
	...
	var APP_HIDE_SPINNER = 'APP_HIDE_SPINNER';
	exports.APP_HIDE_SPINNER = APP_HIDE_SPINNER;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var buildEffectHandler = function buildEffectHandler(handlers) {
	  return function (dispatcher, effect) {
	    (0, _immutable.Map)(handlers) // just wrap it in immutable map, we would like to use the fance methods like the filter
	    .filter(function (handler, effectType) {
	      return effectType === effect.type;
	    }).forEach(function (handler) {
	      return handler(effect.payload, dispatcher);
	    });
	  };
	};
	
	exports['default'] = buildEffectHandler({});
	module.exports = exports['default'];
